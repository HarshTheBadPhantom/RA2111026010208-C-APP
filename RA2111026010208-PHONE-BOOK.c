...
{
    system("cls");
    printf("\t\t```````````````Welcome to Phonebook manager ````````````````");

    printf("\n\n\t\t\t  Selcect from the menu\t\t\n\n");
...
void listrecord()
{
    ...
    {
        printf("\n\n\n YOUR RECORD IS\n\n ");
        printf("\nName=%s\nAdress=%s\nFather name=%s\nMother name=%s\nMobile no=%ld\nSex=%s\nE-mail=%s\nPincode no=%s", p.name, p.address, p.father_name, p.mother_name, p.mble_no, p.sex, p.mail, p.citision_no);
